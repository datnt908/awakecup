...
import { ProductAPIsService, CategoryAPIsService } from '../utils/apiCalling';
...
class MenuView extends Component {
  ...
  getProductsByCategory() {
    ProductAPIsService.search('', '', '', '', '', '', '', '', '', 'Title ASC', 1, 50)
      .then(response => response.json())
      .then(json => {
        this.setState({ products: json.result.items });
      })
      .catch(error => console.log(error));
  }

  getAllCategories() {
    CategoryAPIsService.search('', '', '', 'ID ASC', 1, 10)
      .then(response => response.json())
      .then(json => {
        this.setState({ categories: json.result.items });
      })
      .catch(error => console.log(error));
  }
